"""
Test: Customer C — Product 0003, Winter, Volume 90, EMEA
...
def processed_calculator():
    """
    Calculator for Customer C with product 0003 at individual rate 1,000.
...
class TestCustomerCScenario:
    """Customer C: NON-STRATEGIC + INDIVIDUAL, product 0003, winter, vol 90, EMEA"""

    def test_receivable_before_discount(self, processed_calculator):
        """vol 90 × rate 1,000 = 90,000 (no discounts apply)"""
    ...
    def test_discount_amount(self, processed_calculator):
        """No season, volume, or category discounts → discount amount = 0"""
        result = processed_calculator.df.iloc[0]['Total discounts']
        expected = 0.0
        try:
            assert result == expected
            print(f"\nPASSED: Total discounts = {result}")
        except AssertionError:
            print(f"\nFAILED: Total discounts — expected {expected}, got {result}")
            raise

    def test_receivable_after_discount(self, processed_calculator):
        """90,000 − 0 discount = 90,000"""
    ...
    def test_total_receivable(self, processed_calculator):
        """90,000 receivable after discount + 10% EMEA loading (9,000) = 99,000"""
...
class TestDiscountRuleBreakdown:
    ...
    def test_emea_region_loading_is_ten_percent(self):
        """Rule E: EMEA adds 10% on receivable after discount (90,000 × 10% = 9,000)"""
        result = self._calc.calculate_rule_e_region_loading('EMEA', 90_000.0)
        try:
            assert result == 9_000.0
            print(f"\nPASSED: Rule E region loading (EMEA, base=90,000) = {result}")
        except AssertionError:
            print(f"\nFAILED: Rule E region loading — expected 9,000.0, got {result}")
            raise
